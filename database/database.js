...
class Database {
  ...
  async updateUser(telegramId, updates) {
    try {
      const users = await fs.readJson(this.usersPath);
      if (!users[telegramId]) throw new Error('User not found');
  ...
  async getAlphaWallets(telegramId) {
    try {
      ...
      if (!user || !user.alphaWallets) return [];
  ...
  async removeAlphaWallet(telegramId, walletId) {
    try {
      ...
      if (users[telegramId]?.alphaWallets) {
        users[telegramId].alphaWallets = users[telegramId].alphaWallets.filter(id => id !== walletId);
      }
