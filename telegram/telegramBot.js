...
• /done - Finish`, {
    ...
    async showRecentTrades(ctx, user, isCallback = false) {
        const trades = await database.getUserTrades(user.id, 10);
        
        if (trades.length === 0) {
            const message = '📝 No trades found.';
            const keyboard = new InlineKeyboard().text('◀️ Back', 'trades_menu');
        ...
        const keyboard = new InlineKeyboard()
            .text('🔄 Refresh', 'recent_trades')
            .text('◀️ Back', 'trades_menu');
    ...
    async showPnL(ctx, user, isCallback = false) {
        const trades = await database.getUserTrades(user.id, 100);
        
        if (trades.length === 0) {
            const message = '📝 No trades to calculate PnL.';
            const keyboard = new InlineKeyboard().text('◀️ Back', 'trades_menu');
...
📊 Mode: ${monitorStatus.pollingMode ? 'Polling' : 'WebSocket'}
